#!/usr/bin/env python3
"""
Gmail MCP Server - Dedicated server for Gmail tools.
This server provides Gmail-specific tools via MCP protocol.
"""
...
def create_gmail_mcp_server():
    ...
    try:
        # Create MCP server
        mcp_server = FastMCP("Gmail MCP Server")
        
        # Create and register Gmail service
        gmail_service = GmailService()
        gmail_service.register_tools(mcp_server)
        
        logger.info("✅ Gmail MCP server created successfully")
        logger.info(f"✅ Registered {gmail_service.tool_count} Gmail tools")
        ...
        logger.error(f"❌ Failed to create Gmail MCP server: {e}")
        return None
...
def main():
    ...
    parser.add_argument("--port", type=int, default=9002,
                       help="Port for HTTP transport (default: 9002)")
    ...
    logger.info(f"🚀 Starting Gmail MCP server with {args.transport} transport")
    ...
    try:
        if args.transport == "stdio":
            ...
            logger.info(f"📡 Gmail MCP server listening on http://{args.host}:{args.port}/mcp")
