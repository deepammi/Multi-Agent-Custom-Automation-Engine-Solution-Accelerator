#!/usr/bin/env python3
"""
Test integrated Planner Agent + Gmail Agent workflow using existing backend.

This script tests the complete workflow that should happen when a user submits
a Gmail-related task through the frontend:
1. Task submission to /api/v3/process_request
2. AI Planner analyzes task and generates sequence
3. Planner Agent executes first
4. Gmail Agent executes based on planner's analysis
...
async def test_integrated_workflow():
    """Test the complete integrated workflow using existing backend."""
    
    try:
        logger.info("🚀 Testing integrated Planner + Gmail workflow")
        ...
        # Test task that should trigger Planner -> Gmail workflow
        test_task = "check the last 5 mails and summarize them"
        ...
        if plan_response.status_code == 200:
            ...
            if agents_executed and agents_executed[0] == "Planner":
                logger.info("✅ Planner Agent executed first (correct routing)")
            else:
                logger.warning(f"⚠️ Expected Planner first, got: {agents_executed[0] if agents_executed else 'None'}")
            
            # Check if Gmail agent was executed
            if "Email" in agents_executed or "Gmail" in agents_executed:
