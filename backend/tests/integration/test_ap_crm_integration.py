#!/usr/bin/env python3
"""
AP and CRM Agent Integration Test Script with Real Gemini LLM Calls

This script tests the complete workflow of both AccountsPayable and CRM agents
with real Gemini LLM integration, similar to the Gmail agent integration test.
...
async def test_ap_agent_integration():
    """Test AccountsPayable agent with real Gemini LLM integration."""
    
    print("🔍 AccountsPayable Agent Integration Test with Real Gemini LLM")
    print("=" * 70)
        ...
        for i, test_case in enumerate(ap_test_cases, 1):
            ...
            try:
                ...
                if action_matches and validation_passed and has_required_fields:
                    print(f"   ✅ TEST PASSED - Gemini LLM correctly analyzed the request")
                    passed_tests += 1
                else:
                    print(f"   ❌ TEST FAILED - Gemini LLM analysis did not match expectations")
                    failed_tests += 1
                
                # Note: We're not calling the actual MCP services in this test
                # to avoid dependencies on external services
                print(f"   📝 Note: MCP service calls skipped for testing")
...
async def test_crm_agent_integration():
    """Test CRM agent with real Gemini LLM integration."""
    
    print("\n🔍 CRM Agent Integration Test with Real Gemini LLM")
    print("=" * 70)
...
async def main():
    ...
    print("🚀 Starting AP and CRM Agent Integration Tests with REAL Gemini LLM Calls...")
    print("=" * 90)
    print("Testing complete workflow: User Query → Gemini LLM Analysis → MCP Query Generation")
    print("=" * 90)
    ...
    if overall_success:
        ...
        print(f"   Check Gemini API configuration and network connectivity")
        sys.exit(1)
