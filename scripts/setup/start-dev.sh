...
cd backend
source venv/bin/activate
python -m app.main > ../temp/logs/backend.log 2>&1 &
BACKEND_PID=$!
cd ..
...
for i in {1..30}; do
    ...
    if [ $i -eq 30 ]; then
        echo -e "${YELLOW}⚠ Backend taking longer than expected. Check temp/logs/backend.log${NC}"
    fi
    sleep 1
done
...
cd src/frontend
npm start > ../../temp/logs/frontend.log 2>&1 &
FRONTEND_PID=$!
cd ../..
...
echo ""
echo "Logs:"
echo "  Backend:  tail -f temp/logs/backend.log"
echo "  Frontend: tail -f temp/logs/frontend.log"
echo ""
